...
class TransportFilter(FilterSet):
    transport_type_of_service = filters.CharFilter(label="Тип услуги")
    ...
    transport_category = filters.CharFilter(
        method="filter_by_transport_category", label="Категория транспорта"
    )
    transport_brand = filters.CharFilter(
        method="filter_by_transport_brand", label="Марка транспорта"
    )
    transport_model = filters.CharFilter(
        method="filter_by_transport_model", label="Модель транспорта"
    )
    transport_engine_type = filters.CharFilter(
        method="filter_by_transport_engine_type", label="Тип двигателя"
    )
    transport_drive_type = filters.CharFilter(
        method="filter_by_transport_drive_type", label="Вид привода"
    )
    ...
    transport_transmission_type = filters.CharFilter(
        method="filter_by_transport_transmission_type", label="Тип коробки передач"
    )
    transport_body_type = filters.CharFilter(
        method="filter_by_transport_body_type", label="Тип кузова"
    )
    transport_condition = filters.CharFilter(
        method="filter_by_transport_condition", label="Состояние транспорта"
    )
