...
export const saveTokensAuthToCookie = (
    accessToken?: string,
    refreshToken?: string
) => {
    if (accessToken)
    ...
    if (refreshToken)
        Cookies.set('refresh_token', refreshToken, { expires: 15 }); //хранить в httpOnly cookie (хз как передать в тело запроса, чтобы получить новый access)
};
/*
1/1440 - 1 минута
