...
class RateAPIView(mixins.ListModelMixin, GenericViewSet):
    ...
    def change_rate(self, request, *args, **kwargs):
        ...
        if to_user_pk == from_user_pk:
            return Response(status=status.HTTP_400_BAD_REQUEST, data={"message": "You can't rate yourself."})
        else:
            serializer = self.get_serializer(data=request.data)
            serializer.is_valid(raise_exception=True)
            if user_rate := self.get_queryset().filter(from_user=from_user_pk, to_user=to_user_pk).first():
                user_rate.rating = serializer.validated_data["rating"]
                user_rate.comment = serializer.validated_data["comment"]
                user_rate.save()
            else:
                user_rate = Rate(
                    from_user=get_user_model().objects.get(pk=from_user_pk),
                    to_user=get_user_model().objects.get(pk=to_user_pk),
