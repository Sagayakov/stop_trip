...
type Drive = 'Передний' | 'Задний' | 'Постоянный полный' | 'Полный подключаемый' | 'Не выбрано'

type TransmissoinType = 'МКПП' | 'АКПП' | 'Робот' | 'Не выбрано'

type BodyType = 'Седан' | 'Хэтчбэк' | 'Лифтбэк' | 'Купе' | 'Кабриолет' | 'Внедорожник' | 'Лимузин' | 'Пикап' | 'Не выбрано'

type Condition = 'Новый' | 'Б/у' | 'Аварийный' | 'На запчасти' | 'Не выбрано';
